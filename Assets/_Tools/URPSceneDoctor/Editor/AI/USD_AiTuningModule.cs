...
namespace URPSceneDoctor.Editor
{
    ...
    public sealed class USD_AiTuningModule : IToolModule
    {
        ...
        private void Propose(USD_Settings settings, USD_LabelCatalogAsset catalog)
        {
            ...
            if (USD_LlmClient.IsEnabled(settings))
            {
                ...
                if (res.success)
                {
                    ...
                    else _proposal.warnings.Add("ParseFailed: fallback used");
                }
                else _proposal.warnings.Add("LLMFailed:" + res.error);
            }
        ...
        private void CreateAndApplyProfile(string path, USD_AiTuningParams p, string curvePreset, USD_SmhHint smh)
        {
            ...
            smhComp.shadows.overrideState = true; smhComp.highlights.overrideState = true;
            var sign = _contrastPairHint == "warm" ? 1f : -1f;
            smhComp.shadows.value = new Vector4(smh.shadowsBias * sign, smh.shadowsBias * -sign, smh.shadowsBias * 0.4f, 0f);
            smhComp.highlights.value = new Vector4(smh.highlightsBias * -sign, smh.highlightsBias * sign, smh.highlightsBias * 0.4f, 0f);
        ...
        private void NormalizeByPolicy(USD_AiTuningVariant v, USD_ImageMetricsAggregate m)
        {
            ...
            if (m.overexposure_ratio >= 0.01f) { v.@params.Bloom_intensity = Mathf.Min(v.@params.Bloom_intensity, 0.3f); v.@params.CA_postExposure = Mathf.Min(v.@params.CA_postExposure, 0f); v.notesSafe("overexposure guard"); }
    ...
    internal static class USD_AiTuningVariantExt
    {
        public static void notesSafe(this USD_AiTuningVariant v, string note) { /* placeholder extension for readability */ }
    }
}
