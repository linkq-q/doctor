    ...
    public static class USD_Loc
    {
        ...
        private static Dictionary<string, (string zh, string en)> BuildFallback()
        {
            return new Dictionary<string, (string zh, string en)>
            {
                ...
                {"tab.batch",("批量采样","Batch Sampler")}, {"tab.ai",("AI 助手","AI Assist")}, {"tab.aituning",("AI 调参","AI Tuning")}, {"tab.metrics",("指标面板","Metrics")}, {"tab.reports",("报告","Reports")}, {"tab.settings",("设置","Settings")},
                ...
                {"aituning.proposeSystemPrompt",("你是技术美术调参助手。严格输出 JSON；仅允许字段 Bloom.intensity/Bloom.scatter/WB.temperature/WB.tint/Vig.intensity/Vig.smoothness/Grain.intensity；输出 variantA/variantB/notes。","You are a technical art tuning assistant. Output strict JSON only using Bloom.intensity/Bloom.scatter/WB.temperature/WB.tint/Vig.intensity/Vig.smoothness/Grain.intensity and return variantA/variantB/notes.")},
                                {"settings.smhMaxOffset",("SMH 偏移上限","SMH Max Offset")},
                {"metrics.overview",("这些指标是从截图计算出的画面体检数据，用来让 AI 在不读图的情况下也能做判断。","These metrics are computed from screenshots so AI can reason with evidence without direct image reading.")},
                {"metrics.latest",("读取最新证据包","Load Latest Evidence Pack")}, {"metrics.capture",("使用当前场景快照","Use Current Scene Capture")},
                {"metrics.before",("Before 指标","Before Metrics")}, {"metrics.after",("After 指标","After Metrics")}, {"metrics.compare",("Before/After 对比","Before/After Compare")},
                ...
                {"settings.dumpRawResponseToFile",("保存原始响应到文件","Dump raw response to file")},
                {"settings.rawResponseDumpPath",("原始响应保存路径模板","Raw response dump path template")},
